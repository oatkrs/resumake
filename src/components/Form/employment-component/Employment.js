            ...
            <div className="panel">
                ...
                <div className={this.state.isOpened === "false" ? "panel-body hidden" : "panel-body"}>
                    <div className="grid-2-col">
                        ...
                        <SimpleInput value= {this.state.jobTitle == "(not-set)" ? this.props.jobTitle : this.state.jobTitle}  handleInputs={this.handleInputs} title={t("form.jobTitle")}  name="Job Title"  />
                        <SimpleInput  value= {this.state.employer == "(not-set)" ? this.props.employer : this.state.employer}  handleInputs={this.handleInputs} title={t("form.employer")}  name="Employer"  />
                        <div className=" grid-2-col">
                            <SimpleInput placeholder="ex : aug 2020"  value= {this.state.begin == "(not-set)" ? this.props.begin : this.state.begin}  handleInputs={this.handleInputs} title={t("form.begin")} name="Begin" />
                            <SimpleInput placeholder="ex : Jan 2021"   value= {this.state.end == "(not-set)" ? this.props.end : this.state.end}  handleInputs={this.handleInputs} title={t("form.end")}  name="End"  />
                        </div>
                    </div>
                    <SimpleTextArea  value= {this.state.description == "(not-set)" ? this.props.description : this.state.description}  handleInputs={this.handleInputs} title={t("form.description")}  name="Description"  />
                </div>
            </div>
        )
    }
